...
export const createFoodController = async (req,res) =>{
  ...
  try {
     ...
     if(!title || !description || !price || !restaurant){
      return res
        .status(500)
        .json({ message: "title, desc and price is required!!" }, { success: false });
     }
     ...
     await food.save();
     res.status(200).json({ message: "New food item created" , food }, { success: false });
...
export const getAllFoodController = async (req, res ) => {
  ...
  try{
    ...
    res.status(200).json({ message: "Food list get successfully" , food }, { success: false });
  }
  catch(error){
    res.status(500).json({ message: error.message }, { success: false });
  }
...
export const getFoodListByRestaurantController = async (req, res) => {
  ...
  res
    .status(200)
    .json(
      { message: "food list get successfully by restaurant ", foodlistByRestaurant },
      { success: true }
    );
...
export const deleteFoodController = async (req,res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "food item deleted successfully " , food }, { success: true });
...
export const updateFoodController = async (req,res) => {
  ...
  const updatedFood = await Food.findById(foodId, {title,
       description,
       price,
       imageUrl,
       foodTags,
       ...
       await food.save();
       res.status(200).json({ message: "Food updated successfully" , food }, { success: false });
  }
  catch(error){
    res.status(500).json({ message: error.message }, { success: false });
  }
